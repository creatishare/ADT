...
- Name: chat-workspace.spec.ts >> mobile tab switching >> artifact tab badge shows count after generation
- Location: tests/e2e/chat-workspace.spec.ts:145:7
...
Call log:
  - waiting for locator('[data-testid="chat-input"]')
    - locator resolved to <textarea rows="2" data-testid="chat-input" placeholder="描述你的关卡需求，或上传题组/知识点文档…" class="w-full resize-none bg-transparent text-sm leading-6 text-[var(--fg-primary)] placeholder:text-[var(--fg-muted)] focus:outline-none"></textarea>
...
> 150 |       .fill("请生成 [test-scenario:success]");
  ...
  227 | 
  228 |     await page
  229 |       .locator(selectors.chatInput)
  230 |       .fill("刷新前第一次请求 [test-scenario:success]");
  231 |     await page.locator(selectors.chatSendButton).click();
  232 |     await expect(page.locator(selectors.chatLoadingState)).toBeHidden();
  233 |     await expect(page.locator(selectors.chatMessageList)).toContainText(
  234 |       "刷新前第一次请求"
  235 |     );
  236 | 
  237 |     await page.reload();
  238 | 
  239 |     await expect(page.locator(selectors.chatMessageList)).toContainText(
  240 |       "刷新前第一次请求"
  241 |     );
  242 |     await expect(page.locator(selectors.artifactActiveTitle)).toBeVisible();
  243 |   });
  244 | 
  245 |   test("新建 clears messages and artifacts but preserves previous session", async ({
  246 |     page,
  247 |   }) => {
  248 |     await page.goto("/");
  249 | 
  250 |     // Original session with content
```